...
/**
 * 查询码工具类 为汉字生成拼音和五笔助记码
 */
public class QueryCodeUtil {
     ...
     * @return
     */
    public static String getQueryCode(final String str) {
        if (StringUtils.isBlank(str)) {
            return StringPool.EMPTY;
        }
        final StringBuilder sb = new StringBuilder();
        try {
            sb.append(PinyinHelper.getShortPinyin(str));
     ...
     * 获取 五笔拼音码
     */
    private static String getWuBiCode(final String str) {
        if (str == null) {
            return StringPool.EMPTY;
        }
        if ("".equals(str.trim())) {
        ...
        for (int i = 0; i < str.length(); i++) {
            final char temp1 = str.charAt(i);
            final int asciicode = temp1;
            // 如果是254以内的ASCII 表示是西文字母和字符，直接显示，无需翻译
            if ((asciicode >= 1) && (asciicode <= 254)) {
                result.append(temp1);
            } else {
                // 循环遍历汉字五笔的字库（按照五笔首字符字符分组）
                for (int j = 0; j < QueryCodeUtil.WB.length; j++) {
                    final char[] dbTmp = QueryCodeUtil.WB[j].toCharArray();
                    // 循环遍历每个英文字母的字库
                    for (int k = 1; k < dbTmp.length; k++) {
                        // 找到
                        if (temp1 == dbTmp[k]) {
                            // 如果在WB最后一行，表明是中文标点符号，直接显示，否则显示对应的五笔首字母(该行的第1个字符)
                            if (j == (QueryCodeUtil.WB.length - 1)) {
